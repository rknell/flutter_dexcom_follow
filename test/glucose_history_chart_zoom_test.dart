...
void main() {
  test('🛡️ REGRESSION: chart window keeps newest data on right', () {
    final total = 100;
    final initial = GlucoseChartWindow.forVisiblePoints(totalPoints: total, visiblePoints: total);
    expect(initial.maxX, (total - 1).toDouble());
    expect(initial.minX, 0.0);
    expect(initial.visiblePoints, total);

    final zoomed = GlucoseChartWindow.forVisiblePoints(totalPoints: total, visiblePoints: 50);
